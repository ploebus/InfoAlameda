...
public class ConfigManager extends EventDispatcher
{
    ...
    private function configService_resultHandler(event:ResultEvent):void
    {
        try
        {
			...
			//Server Instance
			//================================================
			var serverInstance:String=configXML.server.@name;
			if(serverInstance)
			{
				configData.serverInstance=serverInstance;
			}
            ...
            for (i = 0; i < widgetContainerList.children().length(); i++)
            {
                ...
                if (xmlObject.name() == "widgetgroup")
                {
                    ...
                    createWidgets(widgetGroupList.widget, true, widgetGroupList.widget.length(), widgetGroupList.@label, widgetGroupList.@icon,widgetGroupList.@description );
                }
                else
                {
            ...
            function createWidgets(widgetList:XMLList, grouped:Boolean, groupLength:Number = 0, groupLabel:String = null, groupIcon:String = null, groupDescription = null):void
            {
                for (var p:int = 0; p < widgetList.length(); p++)
                {
					...
					var wGroupDescription = groupDescription; //john
					...
					//JOHN
					var wDescription:String = widgetList[p].@description;
                    ...
                    var widget:Object =
                        {
							...
							groupDescription: wGroupDescription,
                            label: wLabel,
                            icon: wIcon,
                            config: wConfig,
                            preload: wPreload,
            ...
            for (i = 0; i < wContainers.length(); i++)
            {
                ...
                function getWidgetList(wgList:XMLList, grouped:Boolean, groupLength:Number = 0, groupLabel:String = null, groupIcon:String = null):void
                {
                    for (j = 0; j < wgList.length(); j++)
                    {
						...
						var wgGroupDescription:String = wgList[j].@description; //JOHN
                        ...
                        var wg:Object =
                            {
                                ...
                                url: wgUrl,
								description: wgGroupDescription //JOHN
                            };
                        contWidgets.push(wg);
